...
% Math mode set \mathnormal as default, we don't abbreviate \mathsf (sans-serif) and \mathtt. 
% Main goals of general part are:
% 1. to simplify the usage of \mathbb, \mathscr, \mathcal, \mathfrak, \bm(= \mathbf)
...
\tcolorboxenvironment{proof}{% add a vertical bar to the left
    blanker,breakable,left=5mm, before skip=10pt,after skip=10pt, borderline west={1mm}{0pt}{red}}
...
\newcommand{\Res}{\mathrm{Res\mathop{}}}    % for residue and resultant if no ambiguity
...
\DeclareMathOperator{\prox}{\symbf{prox}}		% for proximal operator
...
\newcommand{\ddpartial}{\frac{\partial}{\partial x }}		% for partial derivative
...
\newcommand{\SPD}{\Smbb_{++}}		% for Symmetric Positive Definite Matrices
...
\NewDocumentCommand{\expect}{ e{_^} s >{\SplitArgument{1}{|}}m }{%
    \operatorname{\mbbE}% the expectation operator
    \IfValueT{#1}{_{#1}}% the measure of the expectation, in subscript form
    \IfValueT{#2}{^{#2}}% superscript, if present
...
\NewDocumentCommand{\expectvar}{mm}{%
    #1\IfValueT{#2}{\middle\vert #2}%
}
\newcommand{\expectarg}[1]{\left\lbrack#1\right\rbrack}
