...
% Double square brackets ⟦·⟧ (scalable, for jumps across interfaces)
\DeclarePairedDelimiter{\iseg}{\lBrack}{\rBrack}
...
\newcommand{\mbfPsi}{\mathbf{\Psi}}
\newcommand{\mbfOmega}{\mathbf{\Omega}}
% Capital variant-Greek (\Varepsilon, \Vartheta, \Varpi, \Varrho, \Varsigma,
% \Varphi) exists in neither LaTeX nor unicode-math, so \mathbf of it was
% fatal-on-use. Removed -- there is no capital-variant glyph to embolden.
% (The lowercase \mbfvar* forms above are fine: \varepsilon etc. do exist.)
...
%----------------------------------------------------------------------
% Equation numbering with hyperref anchor fix
% Visual display: equations show as "2.1", "2.2", "3.1", etc.
% Hyperref anchors: unique IDs like "equation.2.1", "equation.3.1"
% This prevents duplicate destination warnings while preserving semantic linking
\ifcsdef{chapter}{
    \numberwithin{equation}{chapter}
    % Ensure hyperref anchors include chapter prefix for uniqueness
    \renewcommand{\theHequation}{\thechapter.\arabic{equation}}
}{
    \numberwithin{equation}{section}
    % Ensure hyperref anchors include section prefix for uniqueness
...
%----------------------------------------------------------------------
%% Inline-friendly matrix shortcuts
%----------------------------------------------------------------------
% \smallpmatrix{a & b \\ c & d} — paren-bracketed smallmatrix for use
% inside running text. Avoids the line-spacing blowout that a regular
% \begin{pmatrix}...\end{pmatrix} causes in inline math.
\DeclareRobustCommand{\smallpmatrix}[1]{%
    \left(\begin{smallmatrix}#1\end{smallmatrix}\right)%
}

% \eval — evaluation map (used in functional calculus, universal
% property diagrams, etc.). Typeset as upright "eval".
...
\NewDocumentCommand{\expect}{e{_^} o m}{
    % e{_^}: #1=subscript, #2=superscript (both optional embellishments)
    % o: #3=condition (truly optional, for conditional expectation)
    % m: #4=expression (mandatory)
...
% --- Alternative Dangerous Bend Curves (unused) ---
\endinput
