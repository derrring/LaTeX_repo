% TOC
\setcounter{secnumdepth}{3}
% Default tocdepth: 2 (through subsection). Classes may override
% before \tableofcontents is called — e.g., e_class_thesis.cls sets 1
% (section only). Users may also override in their preamble.
\setcounter{tocdepth}{2}

% --- Table of Contents Commands ---

% --- Custom command to print a ToC title ---
% This is your excellent helper command. It contains all the conditional
% logic to choose the right title style.
\newcommand{\printtoctitle}[1]{%
    \ifcsdef{chapter}{%
        \par\noindent
        \begingroup
        \normalfont\bfseries\large
        \MakeUppercase{#1}\par
        \vspace*{.2\baselineskip}
        \noindent\rule{\columnwidth}{0.5pt}
        \endgroup
        \par\nobreak
        \vspace{2em}
    }{%
        \section*{#1}
    }
}

%-----------------------------------------------------------------------
%\etoctocstyle[⟨kind⟩]{⟨number_of_columns⟩}{⟨title⟩}

% part toc color
% \titlecontents provided by titletoc package [deprecated, use etoc instead]
% Define a custom TOC line style for parts
% Customizing the TOC
%\etocsettocstyle{⟨before_toc⟩}{⟨after_toc⟩}
%\etocsetstyle{⟨levelname⟩}{⟨start⟩}{⟨prefix⟩}{⟨contents⟩}{⟨finish⟩}

% --- Command for a two-column, brief ToC ---
\newcommand{\brieftoc}{%
    \begingroup
    \edef\e@saved@tocdepth{\number\value{tocdepth}}%
    \etocsettocstyle{}{\clearpage}%
    \newgeometry{left=1.5cm, right=1.5cm, top=2.5cm, bottom=2.5cm}
    \ifcsdef{chapter}{
        \setcounter{tocdepth}{1}
        \begin{multicols}{2}
            \printtoctitle{Brief Contents}
            \phantomsection
            \addcontentsline{toc}{chapter}{Brief Contents}
            \e@orig@tableofcontents
        \end{multicols}
    }
    {
        \setcounter{tocdepth}{2}
        \begin{multicols}{2}
            \printtoctitle{Brief Contents}
            \phantomsection
            \addcontentsline{toc}{section}{Brief Contents}
            \e@orig@tableofcontents
        \end{multicols}
    }

    \restoregeometry
    \setcounter{tocdepth}{\e@saved@tocdepth}
    \endgroup
}

% --- Command for a single-column, detailed ToC ---
\newcommand{\detailedtoc}{%
    \begingroup
    \edef\e@saved@tocdepth{\number\value{tocdepth}}%
    \etocsettocstyle{
    }{\clearpage}
    \ifcsdef{chapter}{
        \setcounter{tocdepth}{3}
        \printtoctitle{Detailed Contents}
        \phantomsection
        \addcontentsline{toc}{chapter}{Detailed Contents}
    }{
        \setcounter{tocdepth}{3}
        \printtoctitle{Detailed Contents}
        \phantomsection
        \addcontentsline{toc}{section}{Detailed Contents}

    }
    \e@orig@tableofcontents
    \setcounter{tocdepth}{\e@saved@tocdepth}
    \endgroup
}

% --- Main ToC ---
% Override \tableofcontents so it always prints a proper heading.
% \mytoc is kept as an alias for backward compatibility.
%
% Freeze the gap before etoc changes font size, then reuse the same dimension
% below the final entry. Primitive vertical rules avoid the implicit paragraph
% boxes created by \rule. This remains page-breakable inside a long ToC; only
% the rule-to-entry gaps are protected from a page break. Local single spacing
% keeps the geometry independent of document-level setspace and parskip.
\newdimen\e@fenced@toc@gap
\newcommand{\e@fenced@toc@open}{%
    \par
    \begingroup
    \linespread{1}\selectfont
    \parskip=0pt\relax
    \e@fenced@toc@gap=.6em\relax
    \parindent=0pt\relax
    \hrule height 1pt width \linewidth
    \nobreak\vskip\e@fenced@toc@gap
}
\newcommand{\e@fenced@toc@close}{%
    \par
    \nobreak\vskip\e@fenced@toc@gap
    \hrule height 1pt width \linewidth
    \endgroup
    \par
}

\let\e@orig@tableofcontents\tableofcontents
\renewcommand{\tableofcontents}{%
    \begingroup
    % tocdepth is not reset here — it respects whatever the class or user set.
    \ifcsdef{chapter}{%
        % Book-based classes: \chapter* heading, own page (\clearpage after)
        \etocsettocstyle{\renewcommand{\contentsname}{Contents}}{}%
        \chapter*{\contentsname}%
        \addcontentsline{toc}{chapter}{\contentsname}%
    }{%
        % Article-based classes: fenced TOC body with no heading; flows into body text
        \etocsettocstyle
            {\e@fenced@toc@open}
            {\e@fenced@toc@close\vspace{0.5\baselineskip}}%
    }%
    \e@orig@tableofcontents
    \endgroup
    \ifcsdef{chapter}{\clearpage}{}%
}
\let\mytoc\tableofcontents

% --- Command to set up a local ToC (e.g., at the start of a chapter) ---

% Local (chapter-level) TOC. The wrapper groups etoc's style mutation so a
% local fence cannot change a later global or local ToC.
\let\e@orig@localtableofcontents\localtableofcontents
\newcommand{\e@fenced@localtableofcontents}{%
    \begingroup
    \edef\e@saved@tocdepth{\number\value{tocdepth}}%
    \etocsettocstyle{%
        \setcounter{tocdepth}{3}%
        \e@fenced@toc@open
    }{\e@fenced@toc@close\vspace{0.5\baselineskip}}%
    \e@orig@localtableofcontents
    \setcounter{tocdepth}{\e@saved@tocdepth}%
    \endgroup
}

\newcommand{\setuplocaltoc}{%
    \let\localtableofcontents\e@fenced@localtableofcontents
}

% Backward-compatible usage remains:
%   \setuplocaltoc
%   \localtableofcontents
%-----------------------------------------------------------------------
%\etoctocstyle[⟨kind⟩]{⟨number_of_columns⟩}{⟨title⟩}

% part toc color
% \titlecontents provided by titletoc package [deprecated, use etoc instead]
% Define a custom TOC line style for parts
% Customizing the TOC
%\etocsettocstyle{⟨before_toc⟩}{⟨after_toc⟩}
%\etocsetstyle{⟨levelname⟩}{⟨start⟩}{⟨prefix⟩}{⟨contents⟩}{⟨finish⟩}

% --- Comprehensive TOC Styling with Auto-Width Adjustment ---

% Custom dot fill definition
\def\mydotfill{\unskip\nobreak\leaders\hbox{\hss\ . \hss}\hfill\nobreak\relax}

% Define lengths for consistent spacing
\newlength{\tocrightmargin}
\newlength{\tocpartsep}
\newlength{\tocchapternumwidth}
\newlength{\tocsectionnumwidth}
\newlength{\tocsubsectionnumwidth}
\newlength{\tocsubsubsectionnumwidth}
\newlength{\tocnumwidthplus}
% M14: per-entry effective number width + per-level "does it overflow" thresholds.
\newlength{\e@tocnumeff}
\newlength{\tocchapternumthresh}
\newlength{\tocsectionnumthresh}
\newlength{\tocsubsectionnumthresh}
\newlength{\tocsubsubsectionnumthresh}

% Set fixed lengths
\setlength{\tocrightmargin}{0.5cm}
\setlength{\tocpartsep}{1em}
\setlength{\tocnumwidthplus}{0.5em} % Additional space after numbers

% Additional space after numbers

% --- Pre-calculated Width Based on Patterns ---
% Adjust these patterns based on your document structure
% Uses begindocument/end to ensure fonts are fully loaded for accurate width calculations
\AddToHook{begindocument/end}{%
    % For PARTS (Roman numerals up to VIII)
    \settowidth{\@tempdima}{\normalsize VIII\hspace{1em}}%
    \edef\tocpartnumwidth{\the\@tempdima}%
    %
    % For CHAPTERS (up to 99)
    \settowidth{\tocchapternumwidth}{\small 99\hspace{0.5\tocnumwidthplus}}%
    %
    % For SECTIONS - calculate the indentation needed
    \settowidth{\tocsectionnumwidth}{\footnotesize 99.9\hspace{\tocnumwidthplus}}% Include space for alignment
    %
    % For SUBSECTIONS - calculate the indentation needed
    \settowidth{\tocsubsectionnumwidth}{\footnotesize 99.9.9\hspace{\tocnumwidthplus}}%
    %
    % For SUBSUBSECTIONS - calculate the indentation needed
    \settowidth{\tocsubsubsectionnumwidth}{\footnotesize 99.9.9.9\hspace{\tocnumwidthplus}}%
    % M14: bare worst-string thresholds (no trailing gap). A number wider than
    % its bare template overflows the fixed box; \e@settocnumeff grows only
    % those, leaving every fitting number on its exact former (fixed) width.
    \settowidth{\tocchapternumthresh}{\small 99}%
    \settowidth{\tocsectionnumthresh}{\footnotesize 99.9}%
    \settowidth{\tocsubsectionnumthresh}{\footnotesize 99.9.9}%
    \settowidth{\tocsubsubsectionnumthresh}{\footnotesize 99.9.9.9}%
}

% M14 helper: set \e@tocnumeff to the fixed template width #1 when the actual
% number #2 fits its bare threshold #3, else to the number's natural width plus
% one \tocnumwidthplus, so an over-long number no longer collides with the title.
% (Chapters use a -0.25em trailing kern which absorbs half the added gap, so a
% single full \tocnumwidthplus keeps both the fixed and grown gaps consistent.)
\newcommand{\e@settocnumeff}[3]{%
  \settowidth{\e@tocnumeff}{#2}%
  \ifdim\e@tocnumeff>#3\relax
    \addtolength{\e@tocnumeff}{\tocnumwidthplus}%
  \else
    \setlength{\e@tocnumeff}{#1}%
  \fi}

% --- Part Style ---
\etocsetstyle{part}
{\begingroup\parindent0pt \nobreak}
{\pagebreak[3]\bigskip}
{\normalsize\rmfamily\centering \color{toccolorpart}%
    \etocifnumbered{
        Part \etocnumber{} --
    }{%
}\MakeUppercase{\etocthename}\par\vspace{\tocpartsep}}
{\endgroup}

% --- Chapter Style ---
\etocsetstyle{chapter}
{\begingroup\color{black}}
{\small\parindent0pt }
{\etocifnumbered{%
        \leftskip0em\rightskip\tocrightmargin\parfillskip-\rightskip
        % Print number naturally with consistent space after
        \e@settocnumeff{\tocchapternumwidth}{\etocnumber}{\tocchapternumthresh}%
        \makebox[\e@tocnumeff][l]{\etocnumber}\hspace{-0.25em}
        % Calculate hanging indent for wrapped chapter titles
        \hangindent\e@tocnumeff
        \textsc{\etocthename}\hfill
        \makebox[2.5em][r]{\footnotesize\etocpage}\par
    }{%
        \leftskip0em\rightskip\tocrightmargin\parfillskip-\rightskip
        \textsc{\etocthename}\hfill \footnotesize\etocpage\par
}}
{\endgroup}
% --- Section Style ---
\etocsetstyle{section}
{\begingroup}
{\footnotesize\parindent0pt}
{\etocifnumbered{%
        \leftskip\tocchapternumwidth
        \rightskip\tocrightmargin\parfillskip-\rightskip
        % Print number naturally with consistent space after
        \e@settocnumeff{\tocsectionnumwidth}{\etocnumber}{\tocsectionnumthresh}%
        \makebox[\e@tocnumeff][l]{\etocnumber}%%
        % Calculate hanging indent based on actual number width
        \hangindent\e@tocnumeff
        \etocname\mydotfill
        \makebox[2.5em][r]{\etocpage}\par
    }{%
        \leftskip\tocchapternumwidth\rightskip\tocrightmargin\parfillskip-\rightskip
        \etocname\hfill \etocpage\par
}}
{\endgroup}

% --- Subsection Style ---
\etocsetstyle{subsection}
{\begingroup }
{\footnotesize\rmfamily\mdseries \parindent0pt}
{\etocifnumbered{%
        \leftskip\dimexpr\tocchapternumwidth+\tocsectionnumwidth\relax
        \rightskip\tocrightmargin\parfillskip-\rightskip
        % Print number naturally with consistent space after
        \e@settocnumeff{\tocsubsectionnumwidth}{\etocnumber}{\tocsubsectionnumthresh}%
        \makebox[\e@tocnumeff][l]{\etocnumber}%
        % Calculate hanging indent
        \hangindent\e@tocnumeff
        \etocname\mydotfill
        \makebox[2.5em][r]{\etocpage}\par
    }{%
        % Unnumbered peer: same ancestor-column left skip as the numbered
        % branch above (chapter + section number columns) so the title starts
        % at the same x; only the current-level number box is dropped. Mirrors
        % the subsubsection unnumbered branch, which already uses the full
        % ancestor width.
        \leftskip\dimexpr\tocchapternumwidth+\tocsectionnumwidth\relax
        \rightskip\tocrightmargin\parfillskip-\rightskip
        \etocname\hfill \etocpage\par
}}
{\endgroup}

% --- Subsubsection Style ---
\etocsetstyle{subsubsection}
{\begingroup}
{\footnotesize\rmfamily\mdseries \parindent0pt}
{\etocifnumbered{%
        \leftskip\dimexpr\tocchapternumwidth+\tocsectionnumwidth+\tocsubsectionnumwidth\relax
        \rightskip\tocrightmargin\parfillskip-\rightskip
        % Print number naturally with consistent space after
        \e@settocnumeff{\tocsubsubsectionnumwidth}{\etocnumber}{\tocsubsubsectionnumthresh}%
        \makebox[\e@tocnumeff][l]{\etocnumber}%
        % Calculate hanging indent
        \hangindent\e@tocnumeff
        \etocname\mydotfill
        \makebox[2.5em][r]{\etocpage}\par
    }{%
        \leftskip\dimexpr\tocchapternumwidth+\tocsectionnumwidth+\tocsubsectionnumwidth\relax
        \rightskip\tocrightmargin\parfillskip-\rightskip
        \etocname\hfill \etocpage\par
}}
{\endgroup}

% --- Appendix Style ---
% Patches \appendix to change ToC style for appendix chapters.
% etoolbox is provided by e_kernel through e_document_layout.

\apptocmd{\appendix}{%
    % Define and apply the new style for appendix chapters in the ToC.
    \etocsetstyle{chapter}
    {\begingroup\color{black}}
    {\small\parindent0pt}
    {%
        \leftskip0em\rightskip\tocrightmargin\parfillskip-\rightskip
        % Define the format for the appendix entry line.
        % We combine "Appendix" with the chapter letter.
        \e@settocnumeff{\tocchapternumwidth}{\appendixname~\etocnumber}{\tocchapternumthresh}%
        \makebox[\e@tocnumeff][l]{\appendixname~\etocnumber}\hspace{-0.25em}%
        \hangindent\e@tocnumeff
        \textsc{\etocthename}\hfill
        \makebox[2.5em][r]{\footnotesize\etocpage}\par
    }
    {\endgroup}%
}{%
    % Code to execute on success (optional, can be left empty)
}{%
    % Code to execute on failure
    \PackageWarning{TOCStyle}{Could not patch the \appendix command!}%
}

% --- Configuration Options ---

% The toccolorpart color is defined once, in c120_color.tex (as cornellred),
% which e_visual loads before this file. A local color source here would be a
% silent-divergent second owner (the removed one declared dark blue, never
% rendered because c120 wins by load order); it is intentionally absent so a
% broken composition fails loud on the color rather than rendering a
% different-but-valid blue.

% Optional: Adjust spacing between TOC entries
% \setlength{\etocbeforechapterskip}{0.5em}
% \setlength{\etocbeforesectionskip}{0.3em}
% \setlength{\etocbeforesubsectionskip}{0.2em}

% Optional: Set TOC depth (what levels to include)
% \setcounter{tocdepth}{3} % Include up to subsubsection

% --- Usage Notes ---
% 1. Place this code in your preamble after loading etoc package
% 2. The spacing after section numbers is now consistent (1em) regardless of number width
% 3. You can adjust the spacing by changing \hspace{1em} to:
%    - \hspace{0.8em} for tighter spacing
%    - \hspace{1.2em} for looser spacing
% 4. The code assumes you're using standard LaTeX sectioning commands
% 5. Make sure to define or remove \color{toccolorpart} based on your needs

% --- Example of Custom Adjustments ---
% For different spacing preferences:

% % For tighter number-title spacing:
% Replace all instances of \hspace{1em} with \hspace{0.7em}

% % For wider number-title spacing:
% Replace all instances of \hspace{1em} with \hspace{1.5em}
\endinput
